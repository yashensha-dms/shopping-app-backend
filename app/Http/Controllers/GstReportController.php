 ...
 * GstReportController
 *
 * Provides a line-by-line GST summary report for a date range.
 *
 * Usage:
...
{
    /**
     * Return a GST report for the given date range.
     *
     * Query params:
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        foreach ($orders as $order) {
            foreach ($order->products as $product) {
                ...
                // pivot->subtotal = taxable value (ex-GST) for orders created after the fix.
                // pivot->tax      = extracted GST amount.
                ...
                // Inclusive line total = taxable + gst (price customer paid, unchanged)
                $inclusiveLine = $taxableValue + $gstAmount;

                // GST rate from product tax relationship
                $gstRate = (float) ($product->tax->rate ?? 0);

                // CGST / SGST split (intra-state: each = half of total GST)
